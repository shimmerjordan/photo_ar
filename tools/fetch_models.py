...
def _advice(url: str, problem: str) -> str:
    ...
    return (
        ...
        f"  地址：{url}\n"
        f"\n"
        f"正常情况下不该走到这一步：模型已随仓库入版本库（models/xfeat.onnx）并打进\n"
        f"Docker 镜像，entrypoint 会用镜像内置副本。走到网络下载说明你在**源码部署**\n"
        f"而本地没有那份文件。三条出路，任选其一：\n"
        f"  1) 从仓库拿：git checkout models/xfeat.onnx（或重新完整 clone）；\n"
        f"  2) 自己导出（与入库那份逐字节一致，导出是确定性的）：\n"
