 ...
 * 能不能上传。
 *
 * Android 的判据是「media 通道是不是隧道」（隧道有请求体上限，超了 413）。网页没有
 * 通道概念，所以这里的判据换成**服务端有没有明确拒绝**：服务端见到 `CF-Ray` 头时会把
 * 上限收到 95MiB，超了返回 413 并说明原因。**没超就照传** —— 网页的正常路径就是隧道，
 * 按来路一律禁等于这一页整个不存在。
 *
...
export const UPLOAD_LIMIT_NOTE =
  '经 Cloudflare 访问时单个文件上限 95MB，超了会被 413 拒掉。传大视频请连回家里的网络或开 Tailscale。'
