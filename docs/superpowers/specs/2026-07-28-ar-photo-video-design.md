...
4. **Cloudflare Tunnel 不得走视频 —— 这是明文禁止，不是灰区**（2026-07-30 复查官方文档后订正；此条早先写作"ToS 灰区、风险可忽略"，那个说法是错的）：CDN 服务条款明文规定非 Enterprise 套餐**不得**通过 CDN 提供视频或"不成比例的图片、音频、大文件"，要用 Stream / Images / Developer Platform；Cloudflare 保留停用或限制 CDN 使用的权利，且**通知不保证提前**。另有两条硬限制同向：请求体上限 Free/Pro **100MB**、Proxy Read Timeout **125 秒**（非 Enterprise 不可调）。所以媒体通道**首选只有 LAN / Tailscale**：默认配置里 Tunnel 候选的 `prefer` 只含 `api`，`tunnel=true`；`EndpointChoice.select` 只有在**没有任何存活候选声明 media** 时才会兜底落到它（§9.3 有意如此 —— 否则 Tailscale 掉线就等于视频完全播不了）。要零暴露就把那张卡关掉。上传另有两道闸：`uploadAllowed()` 在 media 落到隧道时为假，服务端见 `CF-Ray` 的上传直接 413。流量小（约 4.8GB/月，见 §10.1；改档前 0.9GB/月）不构成许可 —— 赔的是整个账号，包括同一 tunnel 上的其它服务。依据与验证方式见
   [部署文档 §7.1](../../nas-deploy-and-cloudflare.md)。
...
|---|---|---|
| 单次数据量 | 上行 50KB / 下行 <50KB | 5-15MB 下行（§12 改档后）；上传原片可达数百 MB |
...
| 兜底 | LAN / Tailscale | Tunnel（10MB 量级的视频仍无问题，但已不算「小包」，见 §10.1） |
...
第三种场景下媒体走隧道 —— 10MB 量级的短视频仍无问题，见 §1.3 第 4 条。
...
按 §12 的 30 秒 / 1080p / 4Mbps 规格（2026-07-30 改档，原来是 15 秒 720p）：

| 指标 | 数值 |
|---|---|
| 一条视频 | 上限 16.2MiB，实测最坏 14.9MiB，日常素材 5-10MiB |
| 1 万条视频总量（**只有配了视频的才算**） | 按平均 8MiB 估 ≈ **80GB**；全部顶到上限 160GB（四盘位 QNAP 仍容得下，但要留意） |
| 家人每天扫 20 次的流量 | 20 × 8MiB ≈ 160MiB/天 ≈ **4.8GB/月** |
| 对比：同机 Plex 单部电影 | 2-10GB |

改档把带宽账放大了约 5 倍（0.9 → 4.8GB/月），但结论不变：**存储和带宽两个瓶颈仍然
都不存在**，仍在同机 Plex 单部电影的量级以内。网盘直链解决的是"存储不够"或"上行
不够"，本项目两者皆不缺。

> 这条账在 web 版（亲友 200 人）下会重算——那里并发和总量都是另一个量级，见 §16。
...
> **2026-07-30 改档**：原规格 15 秒 / 720p / 1.5Mbps（约 2.8MB）实测偏紧——15 秒
> 装不下一段有头有尾的生活片段。现规格 **30 秒 / 1080p / 4Mbps**，上限约 15MB。
> 权衡见下。

```
ffmpeg -i in.mp4 -t 30 \
  -vf scale=-2:min(1080\,ih) \
  -c:v libx264 -preset veryfast -crf 23 -maxrate 4000k -bufsize 8000k \
  -c:a aac -b:a 128k \
  -movflags +faststart \
  out.mp4
```
...
- 1080p / ≤4Mbps + 音频 128k → 30 秒裸算术 **14.8MiB**；实测最坏（纯噪声源）
  **14.9MiB**——比裸算术略大，因为 maxrate 限的是滑窗均值不是总量，故判据取
  1.1 倍余量 = **16.2MiB**（见 §12.1）。日常素材远低于此
- 时长上限 30 秒，超长入库时截断并警告
- `scale=-2:...` 而非 `-1:...`，保证宽度为偶数（H.264 要求）；用 `min(1080,ih)` 而不是
  写死 1080，避免把矮于 1080p 的源放大（放大只增体积不增清晰度）
- **1080p 是给全屏那条路径的**：AR 模式下视频贴在打印照片大小的 quad 上，720p 就已经
  过采样了；但非 ARCore 机型走 `attachFallback` 全屏播放，web 版同理，那里 720p 看得出来

### 12.1 体积上限是个真判据，不只是个注释

`transcode.MAX_PLAYABLE_BYTES = (maxrate + audio) × 时长上限 × 1.1`，由上面几个常量
推导，不是手写的字面量——常量改过一次之后字面量必然对不上，而对不上是静默的。

它同时是 **pass-through 的判据**：原片同时满足「≤1080p、有 faststart、≤30 秒、
**体积 ≤ MAX_PLAYABLE_BYTES**、宽度为偶数」才跳过转码，`playable_asset_id =
video_asset_id`。

体积这一条是随本次改档补上的，**不能省**：旧判据一个字节都不看，靠
`TARGET_HEIGHT=720` 拦住了一切手机原片（手机拍的都 ≥1080p，一律要转）。目标提到
1080p 之后这层掩护就没了——手机原片正好是 1080p，只是码率 20-25Mbps，是规格的五六倍。
漏掉的表现不是报错，而是把一条 30 秒 90MB 的原片原样发给客户端：认出照片后要等半分钟
才出画，服务端、客户端、日志都不报任何错。

### 12.2 转码产物的存放

转码后的播放版写入**服务自有数据目录**（如 `/share/photo-ar/playable/`），不写回用户的照片/视频目录。原片保持原样不动。

### 12.3 硬件编码：VAAPI，不是 QSV

先看软编到底有多慢——本机 Docker（`--cpus=3`，与 compose 一致）转同一条 34s→30s
1080p 高噪声源，只换 preset：

| preset | 本机耗时 | 产物 | 折算 N5095（÷3.1 单核比） |
|---|---|---|---|
| slow | 89.2s | 14.84MiB | **≈ 4.6 分钟/条** |
| medium | 50.7s | 14.84MiB | ≈ 2.6 分钟/条 |
| fast | 33.8s | 14.90MiB | ≈ 1.7 分钟/条 |
| **veryfast（默认）** | **18.2s** | 14.66MiB | **≈ 56 秒/条** |
| ultrafast | 13.3s | 15.36MiB | ≈ 41 秒/条 |

体积几乎不动是因为**高码率源下 `-maxrate` 先撞上，`-crf 23` 根本没约束到**，所以慢
preset 换来的是同码率下的画质，不是更小的文件；而时间差是 5 倍。入库的 attach 是
**同步 HTTP 请求**（要等转码完才回 201），slow 折算后一条 4.6 分钟，本身就会撞客户端
超时——所以软编默认档定在 veryfast，不是 slow。想要画质就把 `video_preset` 调回
medium/slow，代价就是上表。

挂核显后走 VAAPI，这个 preset 完全不生效（硬编不吃 x264 preset），预期再快一个量级。

**不要用 `h264_qsv`**，这条是查证过的，不是选择题：

| 事实 | 后果 |
|---|---|
| 镜像基于 Debian trixie，ffmpeg 链 `libvpl.so.2`（oneVPL） | QSV 走 oneVPL 运行时 |
| oneVPL 的 GPU runtime `libmfx-gen1.2` 只覆盖 **Gen12+** | N5095 = Jasper Lake = **Gen11**，不在覆盖内 |
| Gen11 需要已弃用的 Media SDK 运行时，而 trixie 里 `intel-media-sdk` / `libmfx1` / `libmfxhw64` **三个包都不存在** | 装不上 |
| 实测：容器里 `ffmpeg -encoders` 列得出 h264_qsv，真跑报 `Error creating a MFX session: -9` | 编进去 ≠ 跑得动 |
| `intel-media-va-driver`（iHD）覆盖 **Gen8+**，含 Jasper Lake | **`h264_vaapi` 是这台机器上唯一走得通的硬编** |

实现要点：

- `video_encoder: "auto"`（默认）探测不到核显就**静默回退 libx264**——慢十倍也比入库
  全线失败好；显式写 `"h264_vaapi"` 则探测失败直接报错，这是部署时验证硬编到底有没有
  生效的唯一可靠手段（否则「以为在用硬编、其实全程软编」只能靠掐表发现）
- 探测方式是**真编一帧**，不是查 `ffmpeg -encoders`（见上表最后两行）
- 硬编必须走 **VBR 并显式限 maxrate/bufsize**，不能用 CQP：VAAPI 没有 CRF，而 CQP 的
  体积随内容跑，噪声大的片子会冲过上限 → 产物自身不合规 → 下次重建又判要转码，转出来
  还是不合规
- 缩放留在 CPU（`scale` 后接 `format=nv12,hwupload`），只把编码搬到 GPU
- ⚠️ **本地无法验证**：开发机只有 NVIDIA 显卡（`/sys/class/drm/renderD128/device/driver
  → nvidia`），Intel 硬编这条路径在 QNAP 上首次运行时才会真正跑到。验证步骤见部署文档。
...
| 隧道上跑视频**违反 CDN 服务条款**（明文，非灰区，见 §1.3 第 4 条） | 停用整个账号，同一 tunnel 上的其它服务一起没，且通知不保证提前 | 隧道上只跑 API 小包（上行 ~50KB、下行 <2KB）：Tunnel 候选的 `prefer` 不含 media，上传另有 `uploadAllowed()` + 服务端 `CF-Ray` 413 两道闸。残余暴露只有一种情形 —— LAN 与 Tailscale 双双不通时 media 兜底落到隧道（§9.3 有意保留）；要零暴露就把那张卡关掉 |
