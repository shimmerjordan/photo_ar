...
## 42. 第 19 轮：修好的代码到不了用户手上 —— CDN 覆盖了缓存头

上一轮的两个修复（上传 400、按钮间距）都已经在 `latest` 里，逐字节比过、跑起来实测过。
用户那边**两个都还是旧行为**。

### 42.1 三个能查的地方全都显示正常

容器日志 `版本 sha-2bb5095`（新）、镜像里的文件逐字节一致（新）、`curl` 边缘节点拿到的
文件里有 `uploadName` 和 `.bar2 { margin-top }`（新）。只有那一个浏览器里是旧的。

原因在响应头上：

```
cache-control: max-age=14400      ← 源站发的是 no-cache
cf-cache-status: REVALIDATED
```

Cloudflare 的 **Browser Cache TTL** 覆盖了源站的 `Cache-Control`。源站的策略是对的
（`no-cache` + ETag：每次问一句、没变就 304，零下载成本），到浏览器手上变成"四小时内
连问都不问"。

HTML 不受影响（Cloudflare 默认不缓存 HTML，`/` 是 DYNAMIC，`no-cache` 原样透传），
所以**页面框架是新的、里面的模块是旧的** —— 这个组合让症状更难认：版本号显示是新的。

### 42.2 为什么这不能只靠"把控制台配对"

配对是必须的（`docs/deploy-details.md` 新增一节写了具体怎么配）。但这个失败模式的要害
是**它没有任何迹象**：三个能查的地方都正常，而症状（接口 400、样式没生效）长得像别的
毛病。两轮排查全花在追那些假线索上。

一个部署配置错误，代价是两轮开发时间 —— 那说明缺的不是配置，是**这件事被发现的途径**。

### 42.3 判据：一个跟着包一起变旧的版本号

`staleguard.js` 里的 `BUILD` 由 web-front 在**发这个文件时**替换（版本来自运行时的
`PHOTOAR_VERSION`，而 `public/` 是只读的镜像内容，构建时写不进去）。它跟别的 js
走同一套缓存策略是**故意的**：浏览器手上的包旧了，这个文件也旧，它带的版本号就是旧的。

对照组是 `/api/config` 的 `version` —— 那条接口 `no-store`，路径不带静态后缀，CDN 不会
缓存它，所以一定是新的。

两个不等 ⇒ 浏览器手上的 JS 比服务端旧。**这个推断不会假阳**：两个值由同一个
`PHOTOAR_VERSION` 生成，同一次部署里必然相同。反过来，任何一边拿不到就不判 ——
误报（给一个没问题的用户弹"你的页面是旧的"，而他刷新之后还是那句）比漏报糟得多。

ETag 必须带上版本：镜像里这个文件的 size/mtime 不变而发出去的内容随版本变，沿用普通
静态文件那个 `size-mtime` 的 ETag 会让升级后的浏览器拿到 304、继续用旧版本号 ——
那正好让这个探测器自己失效。有一条测试钉住这个。

### 42.4 修复动作为什么不是 `location.reload()`

普通 reload 对还在 `max-age` 有效期内的资源**一个请求都不发**，刷十次也是旧的。用户
能做的只有长按刷新键硬刷新，而那件事没人知道。

所以按钮做的是：把**这一页实际加载过的**同源 js/css 逐个
`fetch(url, {cache: 'reload'})` —— 那个选项绕过缓存去问源站，并把新内容**写回 HTTP
缓存**，写回之后再 reload 就是新的了。清单取自
`performance.getEntriesByType('resource')`（浏览器自己记的"我加载了这些"），比在代码里
手写一份模块清单准，而且不会随重构过期。

### 42.5 固有局限，必须写下来

**加这个探测的那一版自己救不了自己。** 用户手上如果是"还没有这段代码"的旧包，这段代码
就不会跑；它从下一次部署开始生效。这是这类自检的固有形状 —— 但也正因为如此，它
**不能替代把 CDN 配对**，两件事都要做。

### 42.6 顺带澄清两条日志里的假警报

- `GET /v1/ping -> 401` **不是故障**。`docker/healthcheck.py` 写明了"没配 token 时 401
  就算健康"——它证明的是进程在监听、HTTP 解析通了、路由匹配到了、鉴权层跑过了。
- `[fetch-models] 取不到 XFeat 模型` 对**网页版没有影响**。浏览器端用的是 ORB
  （`recognize/pipeline.js` + OpenCV wasm），库里存的也是 ORB 描述子
  （`ORB_LAYOUT`），服务端那个 xfeat 后端是给已下线的安卓客户端和服务端识别用的。
  不想看那段可以设 `PHOTOAR_FETCH_MODELS=0`。

## 43. 第 20 轮：模型进镜像、上传目录自动收编、跟踪加定期重锚

用户报了三件事：模型还是下不来（他的 NAS 连不上 github.com）、上传上去了但"无法映射"、
以及录屏里贴合慢慢滑走。三件事三个根因，全部修在这一轮。

### 43.1 模型随镜像分发，启动不再需要任何外网

原设计是启动时从本项目 GitHub release（tag models-v1）下载 xfeat.onnx。它在真实部署里
死于两件事的**叠加**：那个 release 一直没发布（404），而且用户的 NAS 在国内根本连不上
github.com（timed out）—— 也就是说**就算发了 release 也没用**。

一个 4.3MB 的文件不值得一条外网依赖。改成：

- `tools/export_models.py` 本地导出（**导出是确定性的**：本轮实测产物 sha256 与
  `EXPECTED_SHA256` 逐字节一致，29a81cef…0455c652）；
- 产物入版本库（`models/xfeat.onnx`）、Dockerfile COPY 进镜像；
- entrypoint 的 `_model_source` 三级优先：`PHOTOAR_MODEL_URL`（用户显式指的永远最大）
  → 镜像内置副本（`file://` URL）→ GitHub release（只剩源码部署会走到）。

`file://` 不是特殊分支：fetch_models 的下载走 urllib，它原生支持 file://，所以内置副本
走的是**同一条 sha256 校验 + 原子落盘路径** —— 镜像里的文件坏了同样会被查出来。

`test_the_expected_sha_matches_the_real_export` 的前提反转了（原 docstring 写着
"仓库里没有 xfeat.onnx"），现在直接钉真文件的 sha256 与字节数。

验证：`docker run --network none` 起容器，模型装上、sha 对、健康。

### 43.2 上传目录不在白名单内 → 自动收编，而不是让每次上传 403

用户 NAS 的真实配置：roots 是 `photos=…/photos、videos=…/videos`，而
`PHOTOAR_UPLOAD_DIR=…/inbox` 是它们的**兄弟目录**。`/v1/upload` 落地前要过 SafeRoots
校验 → **每一次上传都 403 path_denied**。而 403 的响应体刻意不回显路径（免得变成探测
工具），于是用户看到的只有"传不上去"，什么线索都没有。这就是"上传还是不行、无法映射"。

修在 `ServerConfig.from_dict`（env 与 json 两条配置路都过它）：上传目录不被任何根覆盖时
**自动收编成一个根**（名字 upload，撞名让位），并打一条写明后果与消音方法的警告。
收编而不是拒绝启动：配了上传目录的人的意图明确就是"要用上传"，替他把意图补全比让
服务起不来好。resolve() 双边解析跟着 SafeRoots 的语义走（QNAP 的 /share 常是符号链接）。

验证：断网容器 + 用户同款环境变量，上传经完整链路落进 inbox，文件在宿主机上。

### 43.3 贴合滑走：漂移的两种来源都不掉点，所以要定期重锚

录屏帧分析 + 代码排查的结论链：

- 渲染侧自洽：`drawCamera` 与 `imageToNdc` 用同一条 cover 规则（注释里就互相指着）；
- 抓帧等比缩放不裁剪，worker 坐标系与渲染坐标系一致；
- 面片恒等于整张照片（FULL_RECT），比例差裁在源侧 —— 视频不会画出四边形外；
- **于是错位只能来自四边形本身**：LOCKED 后光流跟踪的漂移。

漂移的两种主要来源恰恰都**不掉点**：光流误差的随机游走（每帧替换种子位置，误差无
回零机制）；覆膜反光锁点（高光随手的倾斜连贯移动，status=1、往返一致、RANSAC 里还是
一致共识）。而补种子只按**点数衰减**触发 —— 这两种漂移永远不触发它，错位无限期保持。
录屏里"视频慢慢滑走/歪掉，界面一直显示认出了"就是它。

修法：`REANCHOR_MS = 1500` 定期重锚 —— 点数不掉也回到**外观**（对锁定那张重新做
描述子匹配），把漂移寿命的上界钉在 1.5 秒。成本约 7%~15% 占空比，管线在 worker 里，
卡的是观测节奏不是渲染帧率（quadfilter 的外推盖住空档）。顺带修了 `_lastReseedAt`
初值为 0 导致锁定后第一帧就白付一次重锚的问题（`_seedTracking` 里重置计时）。

golden 测试注入 25px 整体平移（与反光锁点同构：RANSAC 全体内点、光流零位移、往返
校验抓不住）：漂移 0.0222 归一化画幅（肉眼明显）→ 重锚后 0.0011，**拉回 95%**。

### 43.4 "把入库的特征提取搬到手机"为什么不是这么回事

用户猜"映射慢是 NAS 在做特征提取，能不能分散到手机"。拆开看：

- **识别本来就全在手机上**（网页版的立身之本，README 第一句）。服务端不参与扫描。
- 入库（createPhoto）的成本构成：arcoreimg 质量分 + build-db（**原生二进制**，浏览器
  跑不了）、ORB/合成视图自匹配（能搬但不是大头）、视频转码（**ffmpeg + VAAPI 硬编**，
  浏览器更跑不了，而且它才是几十秒里的大头）。
- 入库是**每张照片一次**的管理员操作，不是宾客路径 —— 优化它的收益上限本来就小。
...
## 47. 第 24 轮：相机开关按钮 + 登录后视频预取

用户提的两个优化，各解决一个真实痛点。

### 47.1 相机开关（扫描页）

原来相机失败是这一页的**终局**：整屏错误 + return，唯一出路是刷新 —— 而刷新要把
12MB 引擎重走一遍。权限弹窗被划掉、相机被别的 App 占用，都会落到这里。

现在 HUD 里有一个「关相机/开相机」按钮，开就是一次**全新的 `getUserMedia`**（权限
还能弹的浏览器会重新弹），失败时按钮变成「重新开相机」留在原地。顺带是省电开关。
实现上把相机初始化收进 `openCam()/closeCam()`，渲染循环加了一道 `!st.stream` 兜底
（防 cancel 与已入队的 raf 回调赛跑）。

### 47.2 登录后预取（prefetch.js）

视频是扫描路径上唯一的大件，而它恰好在「认出来了」那个最有仪式感的瞬间才开始下载 ——
现场网络（几十人挤一个 AP）的最差时刻。登录后的安静时间正好把它提前拉完。

- **策略**（模块唯一的策略点，有测试钉住）：宾客全取（授权集就是他会扫到的那几张，
  典型个位数）；管理员取最新 8（他看得到全库，全取装不下也没意义，他扫的基本是刚
  入库那几张）。
- **缓存键是流地址**（`/v1/asset/<id>/stream`），不是票据地址：票据一次性，永远匹配
  不上第二次；流地址稳定，且换视频会换 asset id —— 旧缓存自然失效，不需要作废逻辑。
  预取用 `fetch(credentials)` 直连（票据体系是为 `<video>` 标签发明的，fetch 没有
  那个毛病）。
- **不用 Service Worker**：`<video src>` 确实不查 Cache Storage，但这个项目的播放
  本来就不走它（安卓媒体组件的坑，mp4stream.js）—— 播放是页面自己 fetch + MSE，
  只要在那条 fetch 之前先问一句 `cachedStream()` 就够，SW 的生命周期一个不用背。
- **播放侧**：`playStream` 学会吃 Response；MSE 走不通退 `<video src>` 时 Response
  给不出地址，走 `getFallbackUrl`（现取一张票）—— 退化路径与没有预取时完全一样。
- 克制：串行 + 段间 300ms、`saveData` 不跑、任何失败静默跳过只进 diag、
  清理不在计划内的旧条目（管理员的占用被钉在 N 段以内）。

验证：真实浏览器整链路（登录 → 后台取列表 → 拉真视频流 → Cache Storage），缓存页
显示「新取 1，已在本机 1 段」；无视频照片计划为空；节点侧 10 条策略/退化测试。

### 47.3 跳变门（大角度场景的"啪"一下）

用户真机反馈：大角度下贴合会**跳变**。两个来源正好都是不连续事件：定期重锚的纠正
（角度越大漂移越大，纠正量实测 2%~8% 画幅）和低内点下 RANSAC 的单帧毛刺。而
quadfilter 原本是自适应低通（One-Euro 那一族）—— 它只会处理**连续**信号上的噪声，
一次 5% 画幅的跳在 tauFast=25ms 下基本原样穿过。

跟踪器的标准配方（Kalman 验证门 + 持续性确认；Vuforia/ARCore 系的位姿平滑同理）：

- **验证门**（JUMP_GATE=3.5% 画幅，比较对象是**预测位置**所以连贯快速运动永远不触发）：
  离谱观测先扣下；
- **持续性确认**（JUMP_CONFIRM=2）：下一帧还在同一个新位置才认 —— 单帧毛刺被整个
  吞掉，真纠正多等 ~50ms（不可见）；
- **纠正缓入**（CORRECTION_TAU=120ms）：认下之后滑过去而不是瞬移，滑行期间速度清零、
  外推关闭（纠正是不连续，把它的斜率喂进速度估计会把下一段甩过头）；
- **瞬移门**（TELEPORT=15%）：完全换了位置就干脆切换，从错误位置爬半秒更难看。

验证：7 条新单测（毛刺吞掉/滑不跳/不甩头/瞬移/连贯运动不受影响/参数自洽）；
仿真回归确认连贯运动指标一字未变（滞后 -36%/抖动 -31% 那组还在）。

#### 47.3.1 真机复测抓到门下漏洞 → 来源感知的 `corrected` 标（2026-08-07）

上面那套门上线后真机复测（本地 Docker + adb reverse + CDP 驱动 Edge，暂停叠加视频
后逐帧差分录屏）：静止场景锁定跟踪中，**每约 2 秒仍有一次整面片瞬跳**—— diff 掩码
是标准的"双边缘错位带"，位移约 60px/2400（2.5% 画幅）。根因：这个场景的重锚纠正
是 **2%~2.5% 画幅，在 3.5% 的门下面**。穿过正常路径后，单帧台阶把速度估计瞬间拉满
（k→1、tau→tauFast、外推开到 vel×ahead），一两帧内整步应用。把门压到 2% 以下又会
误伤急加速的连贯运动 —— 用幅度猜来源，天花板就在这里。

正解是**不猜**：重锚纠正的落地帧 pipeline 自己知道（补种子后第一个成功四角），打
`corrected` 标带给滤波器（worker `...out` 透传，scan.js 转成 `observe` 的
`{correction:true}`）。带标观测不论幅度直接进滑行：>15% 照样瞬移，<0.5%（收敛线）
交正常路径，中间全部 120ms 指数滑入。同时滑行退出线从门线（3.5%）改成**收敛线**
（CORRECTION_DONE=0.5%）—— 门线退出会把尾巴交回"快跟"，又变一次小跳；加硬时限
（CORRECTION_MAX_MS=600ms=5τ）兜底：目标持续移动时永远滑不进收敛线，到时交还，
正常滤波自己跟得上。门本身保留，管**没打标**的不连续（RANSAC 单帧毛刺）。

验证：+5 单测（门下纠正逐帧位移 ≤ 剩余 65%、≥3 帧才贴住、微小纠正不进滑行、带标
瞬移、硬时限）共 30 条全过；仿真回归不变；真机 A/B（CDP 在手机 Edge 里跑真实发布
代码，同一串 51ms 节奏 + 每 1.5s 一次 2.5% 纠正的观测）：单帧最大可见位移
**2.08% → 0.87% 画幅**，且不再单帧整步落地。诊断日志里纠正帧带「纠正帧」字样。

### 47.4 贴合准确度提示

判据用**跟踪内点数**（它同时反映角度、距离、遮挡、反光，正是 RANSAC 解稳不稳的
...
## 49. 第 27 轮：视频循环兜底 + 跟踪/重锚补一道行列式复检

### 49.1 视频放完定住不动（看起来像"暂停"）

`loop: true` 早就设在 `<video>` 上，但真正在喂它的是 `mp4stream.js` 的 `MediaSource`
（安卓平台媒体组件那两个坑逼的，见那个文件的模块注释）——`endOfStream()` 之后原生
`loop` 在 Chromium 系上是个老 bug（播完就定在最后一帧，不重新播），跟 `<video src>`
的正常 loop 是两条不同的代码路径。加了显式 `ended` 兜底：跳回 0、`play()`，不管原生
loop 有没有生效；teardown 里对应 `removeEventListener`。

### 49.2 跟踪与重锚都漏了一道检测阶段有的复检

`decideWith`（检测阶段挑候选）会查行列式落在 `[detMin, detMax]=[0.05,20]`（宽区间，
只挡数值上说不通的解，主要是镜像——负行列式）。但 `pipeline._track`（光流跟踪每帧重解
单应）和 `_reseed`（点数触发/定期重锚的整段重匹配）都只看了 `r.h` 非空与内点数，从没看
过这条。大角度意味着相机与照片近乎平行，可用的匹配点又少又弱，RANSAC 在这种输入下
偶尔解出一个数值正常但几何不合理的单应（`normalizedQuad` 只挡得住四角同号为负的退化
情形，挡不住"镶反了但仍在画幅内"这种）——表现是贴合看起来很稳（内点数、跟踪点都够），
但视频镶的位置或朝向是错的，即用户说的"贴错了"。

两处都补了同一条判据，失败时走各自既有的容错路径（`_track` 走 `homography_lost`，
跟丢太多次才放手；`_reseed` 返回 0，按"没补上"处理、继续用旧种子）——不是新的失败
模式，是把已有的容错接到这个漏检口上。`_reseed` 更值得补：它是整段重新跟外观匹配
（不像 `_track` 延续上一帧已经跟对的点），一旦播种用错误单应，后续几帧会带着这个错误
"看起来很稳"地继续，直到下一次 reseed 或跟丢才有机会纠正。

### 49.3 §49.2 的行列式复检回滚了 —— 它挡住的正是它想保护的场景

上线后用户直接反馈：识别贴合好之后视频**立刻**像定住了（不是放完定住，是刚锁定就
这样）。查下来是 §49.2 那条复检自己有结构性缺陷：`[detMin,detMax]` 是拿检测阶段
（几十到几百个、覆盖全图的匹配点）校准出来的宽区间，而跟踪与重锚用的是光流survive
下来的一小撮点（`MIN_TRACK_POINTS=12`、`RESEED_MIN_INLIERS=16`，本来就是"数学下限"
不是"匹配质量好"的门槛）——点数少、分布集中时，RANSAC 解出的单应本身没问题，但它的
**行列式估计**方差会变大，偏出标定区间。而这恰好和它想防的场景是**同一个**场景：
大角度正是点少又弱的时候。也就是这条检查在它最该起作用的地方，误伤概率反而最高。

`MAX_TRACK_MISSES=5`，30fps 下不到 200ms——新锁定后紧接的几帧只要被误判几次就
"放手"，而放手时视频本身不停（`gaveUp` 分支的既有设计），只是四角被摘掉、canvas
上没有面片可画。用户看到的就是"视频在一开始就没动"。

已把 `_track` 与 `_reseed` 里那两处判据整段撤回，两个函数回到 §49.2 之前的样子。
"大角度可能贴错"这个问题依然存在，但**基于行列式幅度的复检不是安全的解法**——
下次如果还要接，应该只查符号（镜像，`det<0`）不查幅度，或者干脆换一个不依赖点数
的判据；这次的教训是没有真机验证就上了一个理论上说得通、实际会自伤的检查。

## 50. 第 28 轮：文档重组、补上大角度复检、真机抓到一个 NaN

### 50.1 「贴错了要再检查」—— 这次用外观证据，不用数值幅度

§49.2 那版（每帧查单应行列式幅度）在 §49.3 被回滚，欠账留到这一轮。重新做的判据完全
换了一层：**`_reseed` 的返回值**。

管线本来每 `REANCHOR_MS`（1.5s）就拿当前帧重新跟参考图做一次外观匹配（`_reseed`）——
"再检查一次"这件事一直在做，只是**返回 0 时被完全忽略**（注释写的是"没补上就继续用旧
种子"）。而 `_reseed` 失败恰恰意味着"照这个位置看过去，长得不像那张照片了"，正是贴错的
直接证据。现在连续 `MAX_REANCHOR_FAILS = 3` 次失败（≈4.5s 外观持续对不上）就放手回到
检测，`reason: 'appearance_lost'`，提示文案是「贴合可能偏了，正对照片重新识别…」。

与 §49.2 的三处关键差别，也是它不会重蹈覆辙的理由：判据是**外观**不是数值幅度；节奏是
1.5 秒一次不是每帧；要求**连续**三次。放手的代价也小 —— 回到检测后照片若还在画面里，
下一两帧就重新锁上（真机上观察到的正是这个）。

### 50.2 真机抓到的 NaN：`_reseedFails` 只在 `reset()` 里初始化

写完之后真机上**完全没反应**：既不放手，诊断行也不打印。根因是计数器只在 `reset()` 里
给了初值，而构造函数没有 —— 第一次锁定期间它是 `undefined`：

- `undefined++` → `NaN`
- `NaN >= 3` **恒假** → 永远不放手
- `NaN` 是 falsy → `...(reanchorFails ? {...} : {})` 把字段整个丢掉 → 诊断行也不打印

**两个后果互相掩护**：功能不生效，而唯一能告诉你"它试过了"的那行日志也不出现，看起来
就像这段代码根本没被执行。`node --check` 不响，单测也覆盖不到（要 opencv wasm）。
构造函数里补上初值，并在那里写明为什么必须重复初始化。

验证方式是**临时把 `_reseed` 硬编成 `return 0`**、重建镜像在真机上跑：修之前 268 帧
一次都没触发；修之后 `重锚失败 1~2/3` 正常计数 → `appearance_lost … 放手 ×4` → 立刻
重新识别锁定。然后撤掉临时补丁，再跑 50 秒真机回归：**31 次重锚全部成功、0 误伤、
0 放手**，视频 `paused:false` 且 `currentTime` 绕回（循环成立）。

### 50.3 视频循环：原生 loop 其实是好的

§49.1 加的 `ended` 兜底在真机上**一次都没触发** —— MSE 这条路上原生 `loop` 工作正常
（`waiting → canplay → playing` 循环，不发 `ended`）。用户那次报的"暂停"实际是 §49.2
的误伤把四角打掉了，视频在底下空转。兜底留着（Chromium 那个 bug 在别的机型/版本上真实
存在，代价是三行），但**它不是那次问题的解药**，回滚才是。

### 50.4 文档重组：一份 732 行的部署文档拆成四份

用户："部署文档需要简明扼要，容易出现的问题或者 FAQ 需要写在最后或者另外用文档写。"

原 `deploy.md` 732 行里混着三类东西：装机主线、管理台使用说明、排障与边角情况。按职责
拆开：

| 文档 | 管什么 | 行数 |
|---|---|---|
| `deploy.md` | 装机主线，每步带「看到什么算成」 | 732 → 318 |
| `usage.md`（新） | 管理台五个页签、宾客视角、素材页 | 108 |
| `faq.md`（新） | 症状对照表 + 逐条展开 | 158 |
| `deploy-details.md` | **只讲为什么**（取舍、实测基线），排障整段移走 | 601 → 533 |

同时清掉现状文档里的原生客户端残留（`docs/README.md` 的过时条目表、`README.md` 的下线
说明、deploy 文档里几处历史旁注）。**保留**的两处是真事实：平台支持声明（"runs on
Android, iOS and HarmonyOS"）和实测机型（"Edge for Android 150"）。

顺带修掉两处**因为删安卓链而变错**的文档：`quality_too_low` 这个拒绝原因随质量分闸门
一起删了（faq 的入库对照表按 `ingest.py` 现在真正会回的码重写），以及备份建议里还列着
不再产出的 `imgdb/`。

写完照抄执行了一遍每条命令，抓到 `grep -n "build:" docker-compose.yml` 会命中注释里那句
"这里刻意没有 `build: .`" —— 照文档做的人会去查一个不存在的问题，已改成锚定行首。
另外写了个脚本核对全部文档间链接与锚点，修掉一个失效锚点。
